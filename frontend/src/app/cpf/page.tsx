...
// A short, human summary of the citizen's issue for the officer. Ask PULSE itself to condense
// the whole chat (frontend-only — reuses the existing /query endpoint, no backend change), and
// fall back to the local heuristic on any failure/timeout so the handoff never stalls.
...
function PulseWidget() {
  ...
  // Custom drag-to-resize. The native CSS `resize` grip is clipped by the panel's rounded /
  // overflow-hidden corner and is dead on touch, so it never actually worked. null ⇒ use the
  // responsive defaults; the first drag switches to explicit px (corner on desktop, top edge
  // on mobile). See onResize* below.
  const [size,setSize] = useState<{w:number;h:number}|null>(null);
  const panelRef = useRef<HTMLDivElement>(null);
  const dragRef = useRef<{x:number;y:number;w:number;h:number;mode:"corner"|"edge"}|null>(null);
  ...
  async function connectOfficer() {
    ...
    setConnecting(true);
    // Condense the chat into an issue summary for the officer while the button shows
    // "Connecting…". summarizeIssue never throws (heuristic fallback), so this can't stall.
  ...
  // Drag-to-resize handlers. The panel is anchored bottom-right (desktop) / bottom (mobile),
  // so the grip lives at the top-left corner (desktop) or top edge (mobile) and the box grows
  // toward the top-left, away from the screen edge. Pointer events cover mouse + touch.
  function onResizeStart(e:React.PointerEvent, mode:"corner"|"edge"){
    const el=panelRef.current; if(!el) return;
    const r=el.getBoundingClientRect();
    dragRef.current={x:e.clientX,y:e.clientY,w:r.width,h:r.height,mode};
    e.currentTarget.setPointerCapture(e.pointerId); e.preventDefault();
  }
  function onResizeMove(e:React.PointerEvent){
    const d=dragRef.current; if(!d) return;
    const clamp=(v:number,min:number,max:number)=>Math.max(min,Math.min(max,v));
    const h=clamp(d.h+(d.y-e.clientY),360,window.innerHeight-100);
    const w=d.mode==="corner"?clamp(d.w+(d.x-e.clientX),300,window.innerWidth-32):d.w;
    setSize({w,h});
  }
  ...
  return (
    <div style={{position:"fixed",bottom:wMobile?12:24,right:wMobile?12:24,zIndex:9999,display:"flex",flexDirection:"column",alignItems:"flex-end",fontFamily:FONT}}>
      {open&&(
        <div ref={panelRef} style={{position:"relative",width:size?size.w:(wMobile?"calc(100vw - 24px)":"min(370px, calc(100vw - 32px))"),height:size?size.h:(wMobile?"calc(100dvh - 92px)":"min(520px, calc(100vh - 100px))"),background:CPF.white,borderRadius:wMobile?18:10,boxShadow:"0 8px 40px rgba(0,0,0,.22)",display:"flex",flexDirection:"column",overflow:"hidden",marginBottom:12}}>
          {wMobile?(
            <div onPointerDown={e=>onResizeStart(e,"edge")} onPointerMove={onResizeMove} onPointerUp={onResizeEnd} onPointerCancel={onResizeEnd} aria-label="Drag to resize chat"
              style={{position:"absolute",top:0,left:"50%",transform:"translateX(-50%)",width:64,height:18,zIndex:6,cursor:"ns-resize",touchAction:"none",display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:5}}>
              <div style={{width:42,height:4,borderRadius:2,background:"rgba(255,255,255,.6)"}} />
            </div>
          ):(
            <div onPointerDown={e=>onResizeStart(e,"corner")} onPointerMove={onResizeMove} onPointerUp={onResizeEnd} onPointerCancel={onResizeEnd} aria-label="Drag to resize chat" title="Drag to resize"
            ...
            {msgs.map((m,i)=>(
              <div key={i} style={{display:"flex",flexDirection:"column",alignItems:m.role==="user"?"flex-end":"flex-start",gap:6}}>
                <div style={{width:"fit-content",maxWidth:"82%",display:"flex",flexDirection:"column",gap:6}}>
                  <div style={{padding:"9px 12px",borderRadius:m.role==="user"?"12px 12px 3px 12px":"12px 12px 12px 3px",background:m.role==="user"?CPF.teal:CPF.white,color:m.role==="user"?CPF.white:CPF.text,fontSize:13,lineHeight:1.55,boxShadow:"0 1px 3px rgba(0,0,0,.08)",display:"flex",flexDirection:"column",gap:2}}>{m.role==="agent"?renderMd(m.content):m.content}</div>
          ...
          <div style={{flexShrink:0,padding:"9px 11px",borderTop:`1px solid ${CPF.border}`,display:"flex",gap:8,background:CPF.white}}>
            <textarea ref={taRef} value={text} onChange={e=>{setText(e.target.value);const el=e.currentTarget;el.style.height="auto";el.style.height=Math.min(el.scrollHeight,120)+"px";}} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();void send();}}} placeholder="Type your question…" rows={2} disabled={busy}
              style={{flex:1,resize:"vertical",overflowY:"auto",maxHeight:200,border:`1.5px solid ${CPF.border}`,borderRadius:6,padding:"7px 9px",fontSize:wMobile?16:13,fontFamily:FONT,outline:"none"}} />
