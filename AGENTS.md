...
- **Citizen name on dashboard**: every inbound Telegram message captures the sender's real name (`from.first_name`+`last_name`, else `@username`) into prefs `display_name` (merged via `upsertUserPrefs` — `src/gateway/telegram.ts`); `doEscalate` stamps it onto the `QueueEntry` (`display_name`, Mongo-persisted by `postToQueue`); the dashboard `buildPerson` (`CpfDashboard.jsx`) shows `entry.display_name` (fallback `deriveName` → "Telegram user 1234"), replacing the old hardcoded "Nathaniel Neo". Financials/NRIC still come from the hash-picked `MOCK_PROFILES`; chat ID is already real (`entry.userId = tg:<chatId>`).
