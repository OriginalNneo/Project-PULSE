...
**`translateText` resilient (HF → LLM fallback).** HF migrated to "Inference Providers" and **de-listed SeamlessM4T-v2** from the free `hf-inference` provider (hard 400). `src/python-bridge/client.ts` `translateText` now tries HF first, then **falls back to the LLM (GLM)** — validated clean zh/ms/ta. This keeps officer↔citizen relay, guided-question text, and UI-string translation working. The translation feature + cache are otherwise untouched. **Soul-free utility calls:** `llmTranslate` (and `llmDetectLanguage`) call `chatComplete(..., { includeSoul: false, disableThinking: true })` — with the soul preamble on, GLM *answered as PULSE* instead of doing the task (e.g. returned a Tamil CPF answer instead of an English translation). `chatComplete` now takes that opts object (default soul-on for the copilot).

**`detectLanguage` (script-first, then HF, then GLM).** The HF detector (`papluca/xlm-roberta`) doesn't cover **ms/ta/ml/pa** — it mislabels them (Malay → "ru", Tamil → "hi"), which silently left `preferred_lang = "en"` so non-English users got English officer replies, prompts, and no dashboard Translate button. `detectLanguage` now (1) resolves the **non-Latin** supported languages deterministically by **Unicode script block** (Tamil/Malayalam/Gurmukhi/Devanagari/CJK), then (2) for Latin text (en vs ms) uses HF, then (3) falls back to a **GLM classification constrained to the 7 supported codes** when HF returns an out-of-set code. Fixes ms/ta/ml/pa detection → everything that keys off `preferred_lang` works.

**Localized escalation handoff.** The "🧑‍💼 Connect to CPF Officer" button label (`officerButtonFor(lang)`, cached per language), the escalation **offer prompt** (`escalation.offerText`, authored in English → `maybeTranslate` in `sendReply`, which now takes `lang`), and the "you're now connected" confirmation all render in the user's language, so the whole handoff follows their starting language — not just the chatbot reply.
...
**Dashboard translation (officer side).** Two directions. **Inbound:** post-escalation relay messages are auto-translated to English (`relayToOfficer`; HF SeamlessM4T is dead → GLM fallback, which works), but the **pre-escalation bot history** (the "Chat History" tab) is stored verbatim in the citizen's language. So citizen bubbles get a translate control. **Live thread** (relay-translated): the relay also stores the citizen's `original` + `original_lang` on the turn, so the bubble shows English by default with a **local "Show original (<lang>)" ⇄ "Show English" toggle** (no fetch). **History tab** (verbatim bot turns) and any relay-failed message get an **on-demand "Translate" button** → `POST /dashboard/translate { text, source, target }` (`dashboard.ts`; reuses `translateText`, auto-detects source when omitted), cached by text. The canned WhatsApp opener is flagged `system` → no control. **Outbound:** the officer types English and it's auto-translated to the citizen's `preferred_lang` on send (already existed); a badge by the reply box states this, and the **actually-sent translation is stored on the officer's `chat_history` turn** (`translated` / `translated_lang`) as a gov audit record of what the citizen received, shown as a "↳ sent in <lang>: …" line under the English bubble.
